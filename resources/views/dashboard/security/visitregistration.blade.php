...
<div class="container">
    <div class="row">
        <div class="col-md-6 offset-md-3">
            <hr>
            <form method="POST" action="{{route('security.visitor.registeration')}}">
                @csrf
                ...
                <div class="form-group">
                    ...
                    <span class="text-red">*</span>
                    <select name="visitor" id="visitor" class="form-control @error('visitor') is-invalid @enderror" required>
                        <option value="">Select Visitor</option>
                        @foreach ($residents as $resident)
                            <option value="{{ $resident->id }}" {{ old('visitor') == $resident->id ? 'selected' : '' }}>
                                {{ $resident->name }}
                            </option>
                        @endforeach
                    </select>
